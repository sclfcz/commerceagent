...
afterAll(() => {
  delete process.env.COMMERCEAGENT_MIGRATION_BACKUP_DIR;
  try {
    db.closeDatabase();
  } catch {
...
describe('database upgrade safety gate', () => {
  test('backs up before destructive migration, preserves audit orphans, and aborts when backup fails', () => {
    ...
    process.env.COMMERCEAGENT_MIGRATION_BACKUP_DIR = migrationBackups;
    db.initDatabase();
    ...
    process.env.COMMERCEAGENT_MIGRATION_BACKUP_DIR = invalidBackupDir;
