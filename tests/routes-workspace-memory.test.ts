...
vi.mock('../src/middleware/auth.ts', () => ({
  authMiddleware: async (c: any, next: any) => {
    c.set('user', {
      id: process.env.COMMERCEAGENT_MEMORY_TEST_USER ?? 'alice',
      username: process.env.COMMERCEAGENT_MEMORY_TEST_USER ?? 'alice',
      display_name: '',
      role: process.env.COMMERCEAGENT_MEMORY_TEST_ROLE ?? 'member',
      status: 'active',
      permissions: [],
...
function asUser(id: string, role: 'member' | 'admin' = 'member'): void {
  process.env.COMMERCEAGENT_MEMORY_TEST_USER = id;
  process.env.COMMERCEAGENT_MEMORY_TEST_ROLE = role;
}
...
afterAll(() => {
  delete process.env.COMMERCEAGENT_MEMORY_TEST_USER;
  delete process.env.COMMERCEAGENT_MEMORY_TEST_ROLE;
  db.closeDatabase();
