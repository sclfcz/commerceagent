...
const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'commerceagent-tool-init-'));
...
describe('CommerceAgent tool initialization', () => {
  ...
  test('real Claude CLI initializes unrestricted builtins and Agent Builder tools', async () => {
    ...
    const stream = runnerSdk.query({
      ...
      options: {
        ...
        env: {
          ...
          ANTHROPIC_AUTH_TOKEN: 'commerceagent-init-test',
          ANTHROPIC_API_KEY: '',
          CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC: '1',
        },
        allowedTools: ['Bash', 'Write', 'Edit', 'Task', 'mcp__commerceagent__*'],
        permissionMode: 'bypassPermissions',
        allowDangerouslySkipPermissions: true,
        settingSources: [],
        strictMcpConfig: true,
        mcpServers: { commerceagent: server },
      },
    });
    ...
    expect(initializedTools).toEqual(
      expect.arrayContaining([
        ...
        'Edit',
        'Task',
        'mcp__commerceagent__agent_profile_prepare',
        'mcp__commerceagent__agent_profile_publish',
      ]),
    );
  }, 20_000);
});
