...
const SHARED_TMP =
  ...
  (() => {
    const d = fs.mkdtempSync(
      path.join(os.tmpdir(), 'commerceagent-routes-groups-owner-'),
    );
...
function asUser(userId: string, role: 'admin' | 'member' = 'member'): void {
  process.env.COMMERCEAGENT_TEST_USER_ID = userId;
...
afterEach(() => {
  delete process.env.COMMERCEAGENT_TEST_USER_ID;
...
describe('POST / workspace Agent membership publication lock', () => {
  ...
  test('slow failing initialization never publishes a group or Agent mapping', async () => {
    ...
    const sourceDir = fs.mkdtempSync(
      path.join(os.homedir(), '.commerceagent-slow-init-source-'),
    );
