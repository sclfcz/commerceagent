...
const REQUIRED_FILES = [
  ...
  'identity.commerceagent.md',
  'bootstrap.commerceagent.md',
];
...
describe('prompts/ files', () => {
  ...
  test('built-in CommerceAgent identity and one-shot owner bootstrap are explicit', () => {
    const identity = fs.readFileSync(
      path.join(PROMPTS_DIR, 'identity.commerceagent.md'),
      'utf-8',
    );
    const bootstrap = fs.readFileSync(
      path.join(PROMPTS_DIR, 'bootstrap.commerceagent.md'),
      'utf-8',
    );

    expect(identity).toContain('你是 **CommerceAgent**');
    ...
    expect(bootstrap).toContain('commerceagent_owner_profile');
