...
const SHARED_TMP =
  process.env.COMMERCEAGENT_HOST_MOUNT_ROUTE_TEST_DIR ??
  (() => {
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'commerceagent-host-mount-route-'),
    );
    process.env.COMMERCEAGENT_HOST_MOUNT_ROUTE_TEST_DIR = dir;
    return dir;
  })();

vi.mock('../src/config.js', async (importOriginal) => {
  const real = (await importOriginal()) as Record<string, unknown>;
  const root = process.env.COMMERCEAGENT_HOST_MOUNT_ROUTE_TEST_DIR!;
  return {
    ...real,
...
vi.mock('../src/middleware/auth.ts', () => ({
  authMiddleware: async (c: any, next: any) => {
    c.set('user', {
      id: process.env.COMMERCEAGENT_TEST_USER_ID ?? 'route-admin',
      username: process.env.COMMERCEAGENT_TEST_USER_ID ?? 'route-admin',
      role: process.env.COMMERCEAGENT_TEST_USER_ROLE ?? 'admin',
      status: process.env.COMMERCEAGENT_TEST_USER_STATUS ?? 'active',
      permissions: [],
    });
    return next();
  },
...
function asUser(
  ...
  process.env.COMMERCEAGENT_TEST_USER_ID = id;
  process.env.COMMERCEAGENT_TEST_USER_ROLE = role;
  process.env.COMMERCEAGENT_TEST_USER_STATUS = status;
}
