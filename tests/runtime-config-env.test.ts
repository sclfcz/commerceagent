...
describe('buildClaudeEnvLines', () => {
  ...
  test('prevents workspace overrides from replacing third-party managed values', () => {
    const lines = buildContainerEnvLines(
      config({ anthropicModel: 'glm-5.2[1m]' }),
      {
        customEnv: {
          ...
          COMMERCEAGENT_FALLBACK_MODEL: 'workspace-fallback',
          PROJECT_ENV: 'kept',
        },
      },
      NO_CUSTOM_ENV,
    ...
    expect(lines).not.toContain('COMMERCEAGENT_FALLBACK_MODEL=workspace-fallback');
  ...
  test('blocks legacy internal workspace path variables from custom env', () => {
    const lines = buildContainerEnvLines(
      ...
      {
        customEnv: {
          COMMERCEAGENT_WORKSPACE_GLOBAL: '/attacker/global',
          COMMERCEAGENT_WORKSPACE_MEMORY: '/attacker/memory',
          PROJECT_ENV: 'kept',
        },
      },
      NO_CUSTOM_ENV,
    ...
    expect(lines).not.toContain('COMMERCEAGENT_WORKSPACE_GLOBAL=/attacker/global');
    expect(lines).not.toContain('COMMERCEAGENT_WORKSPACE_MEMORY=/attacker/memory');
    expect(lines).toContain('PROJECT_ENV=kept');
  });
  ...
  test('blocks container identity and session permission control variables', () => {
    const blocked = {
      ...
      COMMERCEAGENT_HOST_UID: '0',
      COMMERCEAGENT_HOST_GID: '0',
      COMMERCEAGENT_INTERNAL_IDENTITY_MODE: 'direct',
      COMMERCEAGENT_INTERNAL_FUTURE_ROOT_KNOB: '/workspace/group/evil',
      COMMERCEAGENT_PASSWD_FILE: '/workspace/group/passwd',
      COMMERCEAGENT_RECONCILE_SESSION_PERMISSIONS: '1',
      COMMERCEAGENT_MOUNT_PREPARE_MODE: 'recursive',
      COMMERCEAGENT_RUNTIME_USER: 'root',
      COMMERCEAGENT_SESSION_ROOT: '/',
      COMMERCEAGENT_SESSION_PERMISSION_PID: '1',
  ...
  test('injects an authoritative endpoint kind that custom env cannot replace', () => {
    const thirdParty = buildContainerEnvLines(
      ...
      { customEnv: { COMMERCEAGENT_CLAUDE_ENDPOINT_KIND: 'official' } },
      { COMMERCEAGENT_CLAUDE_ENDPOINT_KIND: 'official' },
    );
    ...
    expect(thirdParty).toContain('COMMERCEAGENT_CLAUDE_ENDPOINT_KIND=custom');
    expect(thirdParty).not.toContain('COMMERCEAGENT_CLAUDE_ENDPOINT_KIND=official');
    expect(official).toContain('COMMERCEAGENT_CLAUDE_ENDPOINT_KIND=official');
  });
  ...
  test('clears inherited provider values before host-mode config is applied', () => {
    const env: Record<string, string | undefined> = {
      ...
      COMMERCEAGENT_CLAUDE_ENDPOINT_KIND: 'custom',
      KEEP_ME: 'yes',
    };
    ...
    expect(env.COMMERCEAGENT_CLAUDE_ENDPOINT_KIND).toBeUndefined();
