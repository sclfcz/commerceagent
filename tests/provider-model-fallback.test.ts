...
describe('provider model fallback lifecycle', () => {
  ...
  test('global fallback env is authoritative and empty config removes inherited values', () => {
    const configured = [
      'COMMERCEAGENT_FALLBACK_MODEL=workspace-value',
      'KEEP_ME=yes',
    ];
    applyFallbackModelToEnvLines(configured, 'fallback-model');
    expect(configured).toContain('COMMERCEAGENT_FALLBACK_MODEL=fallback-model');
    expect(configured).not.toContain(
      'COMMERCEAGENT_FALLBACK_MODEL=workspace-value',
    );

    const cleared = ['COMMERCEAGENT_FALLBACK_MODEL=inherited-value', 'KEEP_ME=yes'];
