...
const SHARED_TMP =
  process.env.COMMERCEAGENT_TEST_DATA_DIR ??
  (() => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'commerceagent-cr-mount-'));
    process.env.COMMERCEAGENT_TEST_DATA_DIR = d;
    return d;
  })();
...
vi.mock('../src/config.js', async (importOriginal) => {
  ...
  const dataDir = process.env.COMMERCEAGENT_TEST_DATA_DIR!;
  return {
    ...real,
    DATA_DIR: dataDir,
...
describe('buildVolumeMounts — AgentProfile runtime policy', () => {
  test('does not write retired Agent tool restrictions into the container env', () => {
    ...
    expect(envContent).not.toContain('COMMERCEAGENT_AGENT_DISALLOWED_TOOLS=');
    expect(envContent).not.toContain('COMMERCEAGENT_AGENT_TOOL_POLICY=');
  });
  ...
  test('custom managed MCP is additive to project MCP and excludes unselected managed servers', () => {
    ...
    expect(fs.readFileSync(envFile, 'utf8')).toContain(
      "COMMERCEAGENT_AGENT_MCP_POLICY='custom'",
    );
  });
  ...
  test('host MCP env replaces inherited servers and clears disabled policy', () => {
    const env = {
      COMMERCEAGENT_USER_MCP_SERVERS_JSON: JSON.stringify({ stale: {} }),
      KEEP_ME: 'yes',
    };

    replaceHostMcpServersEnv(env, { github: { command: 'github-mcp' } });
    expect(JSON.parse(env.COMMERCEAGENT_USER_MCP_SERVERS_JSON)).toEqual({
      github: { command: 'github-mcp' },
    });
    expect(env.KEEP_ME).toBe('yes');

    replaceHostMcpServersEnv(env, {});
    expect(env.COMMERCEAGENT_USER_MCP_SERVERS_JSON).toBeUndefined();
    expect(env.KEEP_ME).toBe('yes');
  });
