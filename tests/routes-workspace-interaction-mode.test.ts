...
vi.mock('../src/middleware/auth.ts', () => ({
  authMiddleware: async (c: any, next: any) => {
    const id =
      process.env.COMMERCEAGENT_INTERACTION_TEST_USER ?? 'interaction-owner';
    c.set('user', {
      id,
      username: id,
      role: 'member',
...
function asUser(id: string): void {
  process.env.COMMERCEAGENT_INTERACTION_TEST_USER = id;
}
...
afterEach(() => {
  delete process.env.COMMERCEAGENT_INTERACTION_TEST_USER;
  vi.clearAllMocks();
});
