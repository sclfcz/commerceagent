...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'commerceagent-routes-usage-'));
...
describe('/api/usage contract and isolation', () => {
  ...
  test('main CommerceAgent attribution has a filterable stable sentinel', async () => {
    ...
    expect(body.attributions.agents).toEqual([
      expect.objectContaining({ key: '__main__', name: 'CommerceAgent' }),
    ]);
  });
