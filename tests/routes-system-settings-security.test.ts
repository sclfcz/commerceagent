...
const tmpDir = fs.mkdtempSync(
  path.join(os.tmpdir(), 'commerceagent-system-settings-security-'),
);
  ...
  return {
    ...
    authMiddleware: async (c: any, next: any) => {
      c.set('user', {
        ...
        role: process.env.COMMERCEAGENT_TEST_ROLE ?? 'member',
        status: 'active',
        permissions: JSON.parse(process.env.COMMERCEAGENT_TEST_PERMISSIONS ?? '[]'),
...
function asUser(role: 'admin' | 'member', permissions: string[] = []): void {
  process.env.COMMERCEAGENT_TEST_ROLE = role;
  process.env.COMMERCEAGENT_TEST_PERMISSIONS = JSON.stringify(permissions);
}
...
afterAll(() => {
  delete process.env.COMMERCEAGENT_TEST_ROLE;
  delete process.env.COMMERCEAGENT_TEST_PERMISSIONS;
