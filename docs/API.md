...
- 除明确标记 Public 的接口外，均需要有效的 CommerceAgent Cookie Session。
...
Home Workspace 固定归属当前用户的内置 CommerceAgent。尝试通过
`PATCH /api/groups/:jid/agent-profile` 将 Home 迁移到自定义 Agent 时返回：

```json
{
  "error": "Home Workspace 始终属于内置 CommerceAgent，不能迁移到自定义智能体",
...
`host_path` 是 CommerceAgent/Docker 守护进程所在服务器上的绝对目录，不是浏览器
...
显式选择它，或通过上面的非 Home Workspace 迁移接口建立归属。内置 CommerceAgent 的
...
### CommerceAgent Owner Profile

- `GET /api/workspaces/:jid/owner-profile`
- `PATCH /api/workspaces/:jid/owner-profile`

只允许该用户不可删除的 Home Workspace、内置默认 CommerceAgent AgentProfile 和实际
