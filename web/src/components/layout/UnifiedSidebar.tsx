...
export function UnifiedSidebar({
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="ecommerce-sidebar-shell h-full flex flex-shrink-0">
        <nav className="ecommerce-sidebar-rail w-[4.5rem] h-full flex flex-col items-center py-3 gap-1 flex-shrink-0">
        ...
        <div
          className="ecommerce-sidebar-panel h-full overflow-hidden transition-[width] duration-200 ease-linear"
          ...
          <div className="w-[16.5rem] h-full flex flex-col">
            ...
            <div className="px-3 pb-2 flex-shrink-0">
              <button
                ...
                className="flex min-h-9 w-full items-center justify-start gap-2 rounded-md border border-white/10 bg-white/5 px-3 text-xs font-medium text-white transition-colors hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring cursor-pointer"
              >
                  ...
                  {agentPartitions.primary && (
                    <section aria-labelledby="primary-agent-heading">
                      <h2
                        ...
                        className="px-3 pb-1 pt-1 text-[10px] font-medium tracking-[0.08em] text-slate-400"
                      >
                  ...
                  {agentPartitions.custom.length > 0 && (
                    ...
                    >
                      <h2
                        ...
                        className="px-3 pb-1 text-[10px] font-medium tracking-[0.08em] text-slate-400"
                      >
                        自定义智能体
                      </h2>
      ...
      <ConfirmDialog
        ...
        title="重建工作区"
        message={`确认重建「${clearState.name}」？会永久删除全部聊天记录、上下文、所有子对话及其消息、工作目录文件，以及该工作区的全部 Memory（含版本历史、遗忘记录和 CommerceAgent 称呼偏好）；Home 还会重置首次唤醒状态。关联定时任务会停止并移入回收站，运行历史与持久化目录 (data/extra/) 保留。不可撤销。`}
        confirmText="确认重建"
