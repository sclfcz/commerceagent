...
const READY_FILE = '/run/commerceagent-session-watcher.ready';
const FAILED_FILE = '/run/commerceagent-session-watcher.failed';
...
const generatedRoots = new Map([
  ...
  [
    'chromium',
    {
      path: '/tmp/commerceagent-chromium-profile',
      policy: 'managed',
      normalization: 'node-owned',
    },
  ],
...
function normalizeDescriptor(fd, root, required = false) {
  ...
  if (
    ...
    process.stderr.write(`commerceagent: skipping ${description}\n`);
...
function stop() {
  ...
  try {
    ...
    process.stderr.write(
      `commerceagent permission watcher final scan failed: ${error}\n`,
    );
    process.exit(1);
  }
}
