...
export interface CommerceAgentPromptSources {
...
export function buildCommerceAgentPromptPlan(
  sources: CommerceAgentPromptSources,
): PromptPlan {
  const inputs: PromptBlockInput[] = [];

  if (sources.platformIdentity) {
    inputs.push({
      id: 'identity.commerceagent',
      version: 1,
      scope: 'main',
      owner: 'platform',
      required: true,
      condition: 'built-in default CommerceAgent AgentProfile',
  ...
  if (sources.platformBootstrap) {
    inputs.push({
      id: 'bootstrap.commerceagent',
      version: 1,
      scope: 'main',
      owner: 'platform',
      required: false,
      condition:
        'built-in CommerceAgent Home runtime; each turn follows the host-authoritative owner-profile block',
