/**
 * MCP Tool Definitions for CommerceAgent Agent Runner.
 *
...
export interface McpContext {
  ...
  isHome: boolean;
  isAdminHome: boolean;
  /** Whether this runtime is an interactive session of the main CommerceAgent. */
  agentBuilderEnabled: boolean;
  /** Host admitted Owner Profile capability for built-in CommerceAgent in Home. */
...
export interface CommerceAgentOwnerProfileTurnResult {
...
const OWNER_PROFILE_RESULT_PREFIX = 'commerceagent_owner_profile_result';
...
async function callCommerceAgentOwnerProfile(
  ctx: McpContext,
 ...
 */
export async function acknowledgeCommerceAgentOwnerProfileFirstWake(
  ctx: McpContext,
  leaseToken: number,
  inputTurnId: string,
  ...
  try {
    const result = await callCommerceAgentOwnerProfile(
      ctx,
      'ack_first_wake',
      { leaseToken },
      timeoutMs,
...
export async function fetchCommerceAgentOwnerProfileTurn(
  ctx: McpContext,
  timeoutMs: number = 5_000,
  inputTurnId: string | null | undefined = ctx.currentInputTurnId,
): Promise<CommerceAgentOwnerProfileTurnResult | null> {
  ...
  try {
    ...
    const projection =
      result.projection as unknown as CommerceAgentOwnerProfileProjection;
    ...
    return {
      projection,
      onboardingStatus:
        onboardingStatus as CommerceAgentOwnerProfileTurnResult['onboardingStatus'],
      firstWake:
...
/**
 * Create all CommerceAgent MCP tool definitions for in-process SDK MCP server.
 */
...
Returns null if the current chat is a DM (DMs do not belong to a server). Only works when the current chat is a Discord channel.`,
  ...
  if (ctx.agentBuilderEnabled) {
    ...
    tools.push(
      tool(
        'agent_profile_list',
        "List the current user's top-level Agents and resumable ready drafts. Use this before editing or resuming work so you can identify the target, current version, draft ID, and draft revision. The main CommerceAgent is returned for context but cannot edit itself with Agent Builder.",
        {},
        async () => {
...
Use the skills panel in the UI to find the skill ID (directory name, e.g. "memory", "think").`,
  ...
  const genericMemoryCanonicalKeySchema = z
    ...
    .refine(
      ...
      'Use commerceagent_owner_profile for the reserved owner address field',
    );
