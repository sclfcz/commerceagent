/**
 * Container Runner for commerceagent
...
import { releaseCommerceAgentOwnerIntroductionLease } from './owner-profile-store.js';
import {
  deleteSession,
  getUserById,
 ...
 * 即使 CommerceAgent 项目删除重建，此文件始终存在于宿主机上。
 */
...
function ensureSettingsJson(
  ...
  const projectionFile = path.join(
    ...
    '.commerceagent-native-settings.json',
  );
...
export interface ContainerInput {
  ...
  agentProfile?: {
    ...
    name: string;
    version: number;
    /** Host-authoritative marker for the built-in CommerceAgent AgentProfile. */
    isDefault: boolean;
    identityHash: string;
  ...
  /**
   * True only for an interactive turn in the built-in CommerceAgent Home
   * Workspace before the owner-preferred-address memory has been established.
   */
  commerceAgentBootstrapPending?: boolean;
  /**
   * Host-authoritative initial Owner Profile projection. Present only for an
   * actual-owner interactive turn of built-in CommerceAgent in Home. The runner
  ...
  commerceAgentOwnerProfileEnabled?: boolean;
  /** Host-derived capability flag for main-CommerceAgent interactive sessions. */
...
export function replaceHostMcpServersEnv(
  ...
  delete env['COMMERCEAGENT_USER_MCP_SERVERS_JSON'];
  if (Object.keys(servers).length > 0) {
    env['COMMERCEAGENT_USER_MCP_SERVERS_JSON'] = JSON.stringify(servers);
  }
}
...
export function applyFallbackModelToEnvLines(
  ...
  for (let i = envLines.length - 1; i >= 0; i--) {
    if (envLines[i].startsWith('COMMERCEAGENT_FALLBACK_MODEL=')) {
      envLines.splice(i, 1);
    }
  }
  const fallbackModel = configuredFallbackModel?.trim();
  if (!fallbackModel) return;
  envLines.push(`COMMERCEAGENT_FALLBACK_MODEL=${fallbackModel}`);
}
...
export function buildVolumeMounts(
  ...
  ensureSettingsJson(
    ...
    resolveRuntimeMcpServers(group, agentProfile),
    {
      // The session settings file is CommerceAgent-owned. Always replace this map
  ...
  // authorization, forcing re-auth every IDLE_TIMEOUT (#477). CommerceAgent never
  ...
  for (let index = envLines.length - 1; index >= 0; index -= 1) {
    if (
      ...
      envLines[index]?.startsWith('COMMERCEAGENT_AGENT_TOOL_POLICY=') ||
      envLines[index]?.startsWith('COMMERCEAGENT_AGENT_DISALLOWED_TOOLS=') ||
      envLines[index]?.startsWith('COMMERCEAGENT_AGENT_MCP_POLICY=')
    ) {
  ...
  if (mcpPolicyMode !== 'inherit') {
    envLines.push(`COMMERCEAGENT_AGENT_MCP_POLICY=${mcpPolicyMode}`);
  }
  ...
  // build (e.g. identity.commerceagent.md) is missing inside the container while
...
export function detectContainerHostIdentity(
  ...
  // while CommerceAgent is running; reusing an earlier direct result could bypass
...
export function buildContainerArgs(
  ...
  args.push('-e', `TZ=${tz}`);
  args.push('-e', `COMMERCEAGENT_HOST_IDENTITY_MODE=${hostIdentity.mode}`);
  if (hostIdentity.mode === 'direct') {
    if (isPositiveUnixId(hostIdentity.uid)) {
      args.push('-e', `COMMERCEAGENT_HOST_UID=${hostIdentity.uid}`);
    }
    if (isPositiveUnixId(hostIdentity.gid)) {
      args.push('-e', `COMMERCEAGENT_HOST_GID=${hostIdentity.gid}`);
    }
  }
...
export async function runContainerAgent(
  ...
  try {
    ...
    const agentSuffix = sessionAgentId
      ...
      : '';
    const containerName = `commerceagent-${safeName}${agentSuffix}-${Date.now()}`;
...
export async function runHostAgent(
  ...
  delete hostEnv['COMMERCEAGENT_USER_MCP_SERVERS_JSON'];
  // Provider selection is authoritative. Do not let API settings inherited
  // from the CommerceAgent parent process leak into a host-mode child after the
  ...
  // processes. When commerceagent is started by a background process manager
  ...
  try {
    ...
    if (tierEnv.runnerFallbackModel) {
      hostEnv['COMMERCEAGENT_FALLBACK_MODEL'] = tierEnv.runnerFallbackModel;
    } else {
      delete hostEnv['COMMERCEAGENT_FALLBACK_MODEL'];
    }
    ...
    delete hostEnv['COMMERCEAGENT_AGENT_TOOL_POLICY'];
    delete hostEnv['COMMERCEAGENT_AGENT_DISALLOWED_TOOLS'];
    delete hostEnv['COMMERCEAGENT_AGENT_MCP_POLICY'];
    if (hostMcpPolicyMode !== 'inherit') {
      hostEnv['COMMERCEAGENT_AGENT_MCP_POLICY'] = hostMcpPolicyMode;
    }

    // 路径映射
    hostEnv['COMMERCEAGENT_WORKSPACE_GROUP'] = groupDir;
    hostEnv['COMMERCEAGENT_WORKSPACE_IPC'] = groupIpcDir;
    ...
    // host 模式由 commerceagent 主进程托管（见 permissionMode: 'bypassPermissions'），
