 ...
 * layer. Skills stay out of this list because CommerceAgent governs them with an
...
export interface ClaudeContextPlanArgs {
   ...
   * context. Managed CommerceAgent skills are resolved independently below.
   */
...
const HOST_PROVIDER_ENV_KEYS = new Set([
  'CLAUDE_CODE_OAUTH_TOKEN',
  'COMMERCEAGENT_CLAUDE_ENDPOINT_KIND',
  'COMMERCEAGENT_FALLBACK_MODEL',
...
/**
 * Native Claude settings may contain a provider selected outside CommerceAgent.
 * Claude Code gives settings.env precedence over the child process env, so
 * projecting those values would silently override the provider chosen in the
 * CommerceAgent UI. Keep unrelated native env while leaving provider ownership to
 * buildContainerEnvLines().
 */
function stripHostProviderEnv(
  settings: Record<string, unknown>,
): Record<string, unknown> {
  if (!isPlainRecord(settings.env)) return settings;
  const env = Object.fromEntries(
    Object.entries(settings.env).filter(
      ([key]) =>
        !key.startsWith('ANTHROPIC_') && !HOST_PROVIDER_ENV_KEYS.has(key),
    ),
  );
 ...
 * order. The caller applies CommerceAgent's required env and effective MCP policy
...
export function loadHostClaudeSettings(
  ...
  for (const settingsFile of plan.settingsSourceFiles) {
    ...
    try {
      ...
      // directly. CommerceAgent skips the invalid layer to keep startup usable.
    }
  }
  return stripHostProviderEnv(merged);
}
...
export function buildClaudeContextPlan(
  ...
  const audit: ClaudeContextAudit = {
    ...
    commerceagentPrompt: { totalBytes: 0, files: [] },
    warnings,
  };
...
export function syncHostClaudeContext(
  ...
  // These paths are CommerceAgent-owned projections of the selected native user
