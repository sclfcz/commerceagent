...
/** One durable Home Workspace fact completes CommerceAgent's first-wake ritual. */
export const COMMERCEAGENT_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY =
  'happyclaw.owner.preferred_address';
const COMMERCEAGENT_OWNER_PROFILE_MEMORY_NAMESPACE = 'happyclaw.owner-profile';
...
function isReservedCanonicalKey(value: string | null | undefined): boolean {
  return (
    normalizeNullable(value) === COMMERCEAGENT_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY
  );
}
...
function rejectReservedCanonicalKey(value: string | null | undefined): void {
  ...
  throw new WorkspaceMemoryStoreError(
    'reserved_canonical_key',
    'This canonical key is owned by the CommerceAgent Owner Profile service',
  );
}
...
function isReservedMemoryItem(item: WorkspaceMemoryItem): boolean {
  ...
  return Boolean(
    requireDatabase()
      ...
      .get(item.id, item.id, COMMERCEAGENT_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY),
  );
}
...
function markPlatformMemoryItem(itemId: string, at: string): void {
  requireDatabase()
    ...
    .run(itemId, COMMERCEAGENT_OWNER_PROFILE_MEMORY_NAMESPACE, at);
}
...
function rejectReservedMemoryItem(item: WorkspaceMemoryItem): void {
  ...
  throw new WorkspaceMemoryStoreError(
    'reserved_canonical_key',
    'This memory item is owned by the CommerceAgent Owner Profile service',
  );
}
 ...
 */
export function reconcileCommerceAgentOwnerAddressCanonicalKey(): {
  workspaces: number;
  retiredItems: number;
} {
  const db = requireDatabase();
  return db.transaction(() => {
    const duplicateStores = db
      ...
      .all(COMMERCEAGENT_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY) as Array<{
      store_id: string;
    }>;
    let retiredItems = 0;
 ...
 */
export function enforceCommerceAgentOwnerAddressCanonicalInvariant(): void {
  requireDatabase()
    .prepare(
    ...
    .run(
      COMMERCEAGENT_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY,
      COMMERCEAGENT_OWNER_PROFILE_MEMORY_NAMESPACE,
    );
 ...
 *
 * CommerceAgent may temporarily disable SQLite foreign-key enforcement when a
 ...
 */
export function getCommerceAgentOwnerAddressMemoryItem(workspaceJid: string): {
  store: WorkspaceMemoryStore;
  ...
  const row = requireDatabase()
    ...
    .get(store.id, COMMERCEAGENT_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY) as
    | MemoryItemRow
    | undefined;
 ...
 */
export function mutateCommerceAgentOwnerAddressMemory(input: {
  workspaceJid: string;
  ...
  return db.transaction(() => {
    const { store: initialStore, item: current } =
      getCommerceAgentOwnerAddressMemoryItem(input.workspaceJid);
    const idempotencyKey = normalizeNullable(input.idempotencyKey);
    ...
    if (!current) {
      if (input.clear) {
        throw new WorkspaceMemoryStoreError(
          'revision_conflict',
          'CommerceAgent Owner Profile revision conflict',
