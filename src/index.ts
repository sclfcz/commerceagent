...
} from './commerceagent-bootstrap.js';
import {
  acknowledgeCommerceAgentOwnerIntroduction,
  claimCommerceAgentOwnerIntroduction,
  clearCommerceAgentOwnerPreferredAddress,
  getCommerceAgentOwnerProfileProjection,
  OwnerProfileStoreError,
  setCommerceAgentOwnerPreferredAddress,
  skipCommerceAgentOwnerIntroduction,
  type CommerceAgentOwnerProfileProjection,
...
function buildWebTraceUrl(
  ...
  const base =
    process.env.COMMERCEAGENT_WEB_URL ||
...
export function feedStreamEventToCard(
  ...
  switch (se.eventType) {
    ...
    case 'prompt_suggestion':
      ...
      break;
    case 'context_audit':
      // Context audits describe CommerceAgent/Claude runtime wiring. Keep them in
...
function resolveCommerceAgentOwnerProfileForTurn(input: {
...
}): CommerceAgentOwnerProfileProjection | undefined {
  if (
    !isCommerceAgentBootstrapTurn({
      turnId: input.turnId,
  ...
  try {
    return getCommerceAgentOwnerProfileProjection(workspaceJid);
  } catch {
    return undefined;
  }
}

function isCommerceAgentOwnerProfileRuntimeEligible(input: {
  group: RegisteredGroup;
  profile: AgentProfile | undefined;
  turnId?: string;
  isScheduledTask?: boolean;
  ...
  return isCommerceAgentOwnerProfileRuntimeStructurallyEligible({
    isHome: input.group.is_home === true,
    isDefaultProfile: input.profile?.is_default === true,
    isScheduledTask: input.isScheduledTask,
...
function bindChannelOutboxScope(
  ...
  return activeChannelOutboxScopes.bind(key, {
    ...
    inputTurnId,
    owner: `commerceagent-outbox:${process.pid}:${runtime.runId}`,
  });
}
...
/** A paired/registered direct chat is already bound to one CommerceAgent user.
  ...
  if (isRecovery) {
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro:
          '检测到当前 workspace 切换或更新了顶层 AgentProfile 身份提示词，底层模型 session 已重置。以下是 CommerceAgent 保存的最近对话记录，供你在新身份下延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        maxMessageLength: 700,
        intro:
          '检测到本次因切换 provider 需要使用新的底层模型 session。以下是 CommerceAgent 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
...
async function runAgent(
  ...
  const commerceAgentOwnerProfile = resolveCommerceAgentOwnerProfileForTurn({
    group,
    profile: resolvedAgentProfile,
    turnId,
    isScheduledTask: Boolean(messageTaskId),
  });
  const commerceAgentBootstrapPending =
    commerceAgentOwnerProfile?.onboarding.state === 'pending' ||
    commerceAgentOwnerProfile?.onboarding.state === 'claimed';
  const commerceAgentOwnerProfileEnabled = isCommerceAgentOwnerProfileRuntimeEligible({
    group,
    profile: resolvedAgentProfile,
    turnId,
    isScheduledTask: Boolean(messageTaskId),
    ...
    if (executionMode === 'host') {
      ...
      output = await runAgentWithModelFallback(
        ...
        group,
        {
          ...
          isHome,
          isAdminHome,
          commerceAgentBootstrapPending,
          commerceAgentOwnerProfile,
          commerceAgentOwnerProfileEnabled,
    ...
    for (const {
      ...
      try {
        ...
        const RESULT_FILE_PREFIXES = [
          ...
          'commerceagent_owner_profile_result_',
        ];
...
async function processTaskIpc(
  data: {
    ...
    // Dedicated built-in CommerceAgent Owner Profile facade.
    preferredAddress?: string;
    expectedOnboardingRevision?: number;
    leaseToken?: number;
    // For the main CommerceAgent's conversational Agent Builder
    profileId?: string;
    draftId?: string;
  ...
  switch (data.type) {
    case 'commerceagent_owner_profile': {
      const finish = (payload: Record<string, unknown>): void =>
        writeTaskResult(
          tasksDir,
          'commerceagent_owner_profile',
          data.requestId,
          payload,
        );
      try {
        ...
        switch (data.operation) {
          case 'get': {
            ...
            const claim = claimCommerceAgentOwnerIntroduction({
          ...
          case 'ack_first_wake': {
            ...
            result = acknowledgeCommerceAgentOwnerIntroduction({
              workspaceJid: principal.workspaceJid,
              leaseOwner: data.runnerInstanceId,
          ...
          case 'set':
            result = setCommerceAgentOwnerPreferredAddress({
              workspaceJid: principal.workspaceJid,
          ...
          case 'clear':
            ...
            result = clearCommerceAgentOwnerPreferredAddress({
              workspaceJid: principal.workspaceJid,
              expectedRevision: data.expectedRevision!,
              idempotencyKey: data.idempotencyKey,
              context: principal.context,
            ...
            break;
          case 'skip':
            result = skipCommerceAgentOwnerIntroduction({
              workspaceJid: principal.workspaceJid,
        ...
        logger.warn(
          ...
          'CommerceAgent Owner Profile IPC request failed',
        );
      }
      break;
    }
      ...
      try {
        ...
        if (isFeishuCapabilityMutation(request)) {
          ...
          const mutation = await deliverFeishuCapabilityMutation({
            ...
            request,
            owner: `commerceagent-feishu-capability:${process.pid}:${outboxScope.turnRunId}`,
            execute: () =>
...
async function processAgentConversation(
  ...
  // session was cleared by provider/model switching, inject persisted CommerceAgent
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro: resetForAgentProfile
          ? '检测到当前 workspace 切换或更新了顶层 AgentProfile 身份提示词，当前 agent 的底层模型 session 已重置。以下是 CommerceAgent 保存的最近对话记录，供你在新身份下延续上下文。'
          : '检测到当前 agent 的底层模型 session 是新的（可能因为切换 provider/model 或恢复失败）。以下是 CommerceAgent 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
...
async function startMessageLoop(): Promise<void> {
  ...
  logger.info('commerceagent running');
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          // the notify_channels broadcast (riba2534/commerceagent#559).
...
async function ensureDockerRunning(): Promise<void> {
  ...
  // Kill and clean up orphaned commerceagent containers from previous runs
  try {
    const { stdout } = await execFileAsync(
      'docker',
      ['ps', '--filter', 'name=commerceagent-', '--format', '{{.Names}}'],
      { timeout: 10000 },
    );
 ...
 * by a different CommerceAgent user), re-routing is skipped — the chat stays with
...
function buildOnNewChat(
  ...
  return (chatJid, chatName) => {
    ...
    if (existing) {
      ...
      //      a different CommerceAgent user → do NOT re-route.
      //
      ...
      if (!existing.is_home) {
        ...
        if (previousOwnerStillConnected) {
          ...
          Object.assign(existing, releaseOwner(existing), {
            ...
            // Trust belongs to the previous CommerceAgent user's channel binding,
...
main().catch((err) => {
  logger.error({ err }, 'Failed to start commerceagent');
  process.exit(1);
});
