...
export const CLAUDE_ENDPOINT_KIND_ENV = 'COMMERCEAGENT_CLAUDE_ENDPOINT_KIND';
...
/**
 * Remove provider-owned values inherited from the CommerceAgent parent process.
...
const DANGEROUS_ENV_VARS = new Set([
  ...
  'TEMP',
  'TMP',
  // CommerceAgent 内部路径映射
  'COMMERCEAGENT_WORKSPACE_GROUP',
  ...
  'COMMERCEAGENT_WORKSPACE_GLOBAL',
  'COMMERCEAGENT_WORKSPACE_MEMORY',
  'COMMERCEAGENT_WORKSPACE_IPC',
  'CLAUDE_CONFIG_DIR',
]);
...
function isDangerousEnvKey(key: string): boolean {
  return (
    ...
    key.startsWith('COMMERCEAGENT_SESSION_') ||
    key.startsWith('COMMERCEAGENT_INTERNAL_') ||
    key === 'COMMERCEAGENT_HOST_IDENTITY_MODE' ||
    key === 'COMMERCEAGENT_HOST_UID' ||
    key === 'COMMERCEAGENT_HOST_GID' ||
    key === 'COMMERCEAGENT_PASSWD_FILE' ||
    key === 'COMMERCEAGENT_RECONCILE_SESSION_PERMISSIONS' ||
    key === 'COMMERCEAGENT_MOUNT_PREPARE_MODE' ||
    key === 'COMMERCEAGENT_RUNTIME_USER'
  );
}
...
const DEFAULT_APPEARANCE_CONFIG: AppearanceConfig = {
  appName: 'CommerceAgent',
  aiName: 'CommerceAgent',
