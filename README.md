<p align="center">
  <a href="https://github.com/sclfcz/commerceagent">
    <img src="web/public/icons/commerceagent-icon.png" alt="CommerceAgent Logo" width="128" />
  </a>
</p>

<h1 align="center">CommerceAgent</h1>
...
<p align="center">
  <a href="https://github.com/sclfcz/commerceagent"><img src="https://img.shields.io/badge/GitHub-CommerceAgent-F97316?style=for-the-badge&logo=github" alt="GitHub" /></a>
...
<p align="center">
  <a href="https://github.com/sclfcz/commerceagent"><strong>项目主页</strong></a> ·
  <a href="docs/API.md">API 文档</a> ·
  <a href="docs/ACL-MATRIX.md">权限矩阵</a> ·
  <a href="https://github.com/sclfcz/commerceagent/issues">问题反馈</a>
</p>
...
| 模块           | 主要能力                                                                                |
| -------------- | --------------------------------------------------------------------------------------- |
| **智能体**     | 主 CommerceAgent 对话式创建/编辑、自定义智能体、头像、结构化提示词、AI 优化、版本历史与恢复 |
...
CommerceAgent 使用统一的 `智能体 → Workspace → Runtime Session` 层级：
...
- **内置 CommerceAgent** 是不可删除的平台主 Agent，拥有代码级固定身份，理解
  Workspace、Session、Agent Builder、渠道、任务、Skills/MCP、Provider 和权限
  模型；这些规则不存放在可遗忘的 Memory 中。
- **首次唤醒** 只面向 Home Workspace 中经过宿主验证的实际 owner。CommerceAgent
  用简短的“刚醒来”话语询问称呼，并通过专用 Owner Profile 保存；并发会话使用
  onboarding lease，后续新 Session 或服务重启不会重复表演。清空称呼也不会重置
  onboarding。
- **智能体** 保存长期身份和能力策略。主 CommerceAgent 智能体始终存在，也可以创建代码审查、研究、运维等自定义智能体。
- **对话式智能体构建器** 在主 CommerceAgent 名下的所有 Workspace 主会话与普通 Runtime Session 中生效，允许用户多轮讨论需求、保存并预览智能体草稿，并在后续人类消息逐字回复一次性确认口令后创建或更新智能体；定时任务与 Sub-Agent 不能代替用户发布。
- **Home Workspace** 是不可删除的系统工作区，并永久归属内置 CommerceAgent。
- **自定义 Agent** 创建后没有隐式 Workspace、Session 或 Memory；用户需要显式
  新建 Workspace 或迁移一个非 Home Workspace，避免继承 CommerceAgent 上下文。
...
- 用户 Skills、CommerceAgent MCP 与 Claude Code Plugins；智能体始终拥有完整工具权限。

配置页可以针对具体工作区预览最终生效能力，包括项目 `CLAUDE.md`、`.claude/skills`、项目 MCP、用户能力和同名覆盖来源。

### Skills、MCP 与 Plugins

CommerceAgent 区分不同层级的能力来源：

| 来源                    | 说明                                                              |
| ----------------------- | ----------------------------------------------------------------- |
| **内置能力**            | 随 CommerceAgent 或智能体容器提供的基础工具与 Skills                  |
...
CommerceAgent 可以同时配置多个 Claude Provider，并为新会话按策略选择健康的 Provider。
...
```bash
git clone https://github.com/sclfcz/commerceagent.git
cd commerceagent
make start
```
...
| `make stop`                                     | 停止当前端口上的 CommerceAgent 服务                       |
...
| `make restore FILE=commerceagent-backup-xxx.tar.gz` | 停止服务后恢复指定备份                                |
| `make help`                                     | 查看完整命令列表                                      |
...
`/usr/local/share/commerceagent-tool-versions.txt`，需要回滚时也可以通过 Docker
build args 在发布工作流中指定精确版本。

## 配置与数据

CommerceAgent 优先通过 Web 设置管理配置，不要求用户维护一组庞大的环境变量。
...
| **设置 → 主 CommerceAgent** | 默认智能体的 Skills 与 MCP                            |
...
```mermaid
flowchart LR
    Web["Web / PWA"] --> Core["CommerceAgent Node Service"]
    IM["7 个 IM 渠道"] --> Core
...
主服务始终使用 Node.js 运行，负责认证、路由、队列、调度、渠道连接、Provider 池、用量和持久化。Docker 只用于隔离智能体执行环境，不承载 CommerceAgent 主服务。
...
CommerceAgent 会执行代码和访问第三方消息平台，部署前请理解以下边界：
...
`/usr/local/share/commerceagent-tool-versions.txt`。固定版本的内置 Skills 会在
...
CommerceAgent 是独立开源项目，与 Anthropic 及各消息平台不存在官方隶属、授权或背书关系。相关名称和商标归各自权利人所有。
...
<p align="center">
  如果 CommerceAgent 对你有帮助，欢迎点一个 Star ⭐
</p>
